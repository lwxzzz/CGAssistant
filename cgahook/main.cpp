...
LRESULT UpdateGameWindowTitle(VOID)
{
	...
	if (g_CGAService.IsInGame() && g_CGAService.GetPlayerName() && g_CGAService.GetPlayerName()[0])
		_snprintf(szNewTitle, 256, "%s CGA [%s] (%d窟) #%d", g_BasicWindowTitle, g_CGAService.GetPlayerName(), g_CGAService.GetServerIndex(), g_MainPort - CGA_PORT_BASE + 1);
	else
...
int WINAPI DllMain(HINSTANCE hinstDLL, DWORD fdwReason, LPVOID lpReserved)
{
	if (fdwReason == DLL_PROCESS_ATTACH)
	{
		...
		{
			auto hWnd = FindWindowA(NULL, "“弄鯤繫”隊있틱憩");
			if (hWnd)
			{
				DWORD pid;
